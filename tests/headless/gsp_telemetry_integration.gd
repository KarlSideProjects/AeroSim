...
func _init() -> void:
    ...
    _expect(int(processing.get("wire_serialization_count", 0)) > 0, "wire JSON serialization is measured")
    ...
    _expect(float(processing.get("serialization_p99_usec", 501.0)) < 500.0, "telemetry wire serialization p99 remains below 0.5 ms")
