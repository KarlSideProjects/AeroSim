...
func _init() -> void:
    ...
    if bool(shell_open_failure.get("ok", true)) or not shell_error.contains("OS.shell_open") or shell_error.contains(raw_token) or shell_error.contains(token_url) or shell_error.contains("file://") or shell_error.contains("#") or shell_error.contains("token="):
