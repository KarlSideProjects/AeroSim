...
func _init() -> void:
	...
	_expect(panel.contains("extra: []") and panel.contains("performance.now()") and panel.contains("performance.timeOrigin") and not panel.contains("Date.now()"), "panel uses bounded extras and a comparable monotonic commit clock")
