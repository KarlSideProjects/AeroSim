...
func _init() -> void:
    ...
    _expect(panel.contains("extra: []") and panel.contains("performance.now()") and panel.contains("server_receive_usec") and panel.contains("server_send_usec") and not panel.contains("performance.timeOrigin") and not panel.contains("Date.now()"), "panel uses bounded extras and an NTP-style monotonic commit clock")
    _expect(panel.contains("request_to_native_commit_ms") and panel.contains("native_commit_to_rendered_ack_ms"), "panel names the two measured commit legs honestly")
