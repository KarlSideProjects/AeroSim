...
func test_route_advances_only_by_simulation_delta() -> void:
    ...
    route.advance(29.0, Vector3.ZERO, Vector3.ZERO, 0.0)
    assert_eq(route.snapshot().phase, "hover")
    route.advance(2.0, Vector3.ZERO, Vector3.ZERO, 0.0)
    assert_eq(route.snapshot().phase, "low_pass")


func test_route_starts_with_a_safe_ascent_then_visits_every_recording_phase() -> void:
    var route := DemoFlightRoute.new()
    ...
    for second in range(181):
        var state: Dictionary = route.advance(1.0 if second > 0 else 0.0, spawn, Vector3.ZERO, 0.0)
...
func test_route_keeps_the_approved_three_minute_phase_boundaries() -> void:
    ...
    assert_eq(route.advance(30.0, Vector3.ZERO, Vector3.ZERO, 0.0).phase, "low_pass")
    assert_eq(route.advance(20.0, Vector3.ZERO, Vector3.ZERO, 0.0).phase, "orbit")
    assert_eq(route.advance(65.0, Vector3.ZERO, Vector3.ZERO, 0.0).phase, "climb")
    assert_eq(route.advance(20.0, Vector3.ZERO, Vector3.ZERO, 0.0).phase, "return")
    assert_eq(route.advance(30.0, Vector3.ZERO, Vector3.ZERO, 0.0).phase, "land")
    assert_true(bool(route.advance(15.0, Vector3.ZERO, Vector3.ZERO, 0.0).complete))
...
func test_route_waits_for_the_drone_to_reach_spawn_before_completing() -> void:
    var route := DemoFlightRoute.new()
    route.start(Vector3.ZERO)

    var landing: Dictionary = route.advance(180.0, Vector3(10.0, 5.0, 10.0), Vector3(2.0, 0.0, 0.0), 0.0)
    assert_eq(landing.phase, "land")
    assert_false(bool(landing.complete))

    assert_true(bool(route.advance(0.0, Vector3.ZERO, Vector3.ZERO, 0.0).complete))
...
func test_route_orbits_the_control_tower_with_clearance_and_a_high_pass() -> void:
    ...
    var low_pass: Dictionary = route.advance(49.9, spawn, Vector3.ZERO, 0.0)
    ...
    assert_gte(orbit_horizontal.distance_to(control_tower), 14.0)
    assert_lte(orbit_horizontal.distance_to(control_tower), 25.0)
    assert_lte(orbit_horizontal.length(), 80.0)

    var climb: Dictionary = route.advance(68.0, spawn, Vector3.ZERO, 0.0)
    assert_eq(climb.phase, "climb")
    assert_gt(climb.target_position.y, 18.0)
    assert_lte(climb.target_position.y, 20.0)
...
func test_east_orbit_segment_climbs_above_the_terrain_wall() -> void:
    var route := DemoFlightRoute.new()
    route.start(Vector3.ZERO)
    ...
    for second in range(181):
        ...
        if target.x >= 45.0 and target.z >= 45.0:
            assert_gte(target.y, 15.0)
