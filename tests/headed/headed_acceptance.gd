...
## Terrain Range ground samples. The flight field is authored onto the 79.7 m
## mesa of the upstream Terrain3D valley; see `scripts/author_terrain3d_range.gd`
## for how each surface below is painted.
const TERRAIN_RANGE_GRASS_SAMPLE := Vector3(540.0, 0.0, -760.0)
const TERRAIN_RANGE_SOIL_SAMPLE := Vector3(528.0, 0.0, -790.0)
const TERRAIN_RANGE_ROCK_SAMPLE := Vector3(520.0, 0.0, -790.0)
const TERRAIN_RANGE_RELIEF_SAMPLE := Vector3(578.0, 0.0, -842.0)
...
func _run() -> void:
	...
	var terrain_soil_sample: Vector3 = terrain_range_data.get_texture_id(TERRAIN_RANGE_SOIL_SAMPLE) if terrain_range_data != null else Vector3(-1.0, -1.0, -1.0)
	var terrain_rock_sample: Vector3 = terrain_range_data.get_texture_id(TERRAIN_RANGE_ROCK_SAMPLE) if terrain_range_data != null else Vector3(-1.0, -1.0, -1.0)
