...
func _run() -> void:
	...
	runtime.quick_fly()
	await _settle(10)
	_expect(runtime.screen == "controller_confirmation", "Quick Fly re-confirms a replacement known controller")
	if confirm_button != null:
		_click(confirm_button)
	await _settle(10)
	_expect(runtime.screen == "preflight", "replacement controller confirmation returns to preflight")
