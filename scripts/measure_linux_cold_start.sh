...
frame_marker_path="${AEROSIM_COLD_START_FRAME_MARKER:-${report_path%.json}.frame.json}"
frame_marker_path="$(realpath -m "$frame_marker_path")"
rm -f "$report_path"
rm -f "$screenshot_path"
rm -f "$frame_marker_path"
started_ns="$(date +%s%N)"
"$release_bin" -- \
    --aerosim-cold-start-report "$report_path" \
    --aerosim-cold-start-frame-marker "$frame_marker_path" \
    --aerosim-cold-start-screenshot "$screenshot_path" \
    --aerosim-cold-start-visible-seconds "$visible_seconds" &
release_pid="$!"
frame_deadline=$((SECONDS + max_seconds))
report_deadline=$((SECONDS + max_seconds + visible_seconds + 5))
frame_seen_ns=""
report_seen_ns=""

while kill -0 "$release_pid" 2>/dev/null; do
    if [ -z "$frame_seen_ns" ] && [ -s "$frame_marker_path" ] && python3 -m json.tool "$frame_marker_path" >/dev/null 2>&1; then
        frame_seen_ns="$(date +%s%N)"
    fi
    if [ -s "$report_path" ] && python3 -m json.tool "$report_path" >/dev/null 2>&1; then
        report_seen_ns="$(date +%s%N)"
        break
    fi
    if [ -z "$frame_seen_ns" ] && [ "$SECONDS" -ge "$frame_deadline" ]; then
        kill "$release_pid" 2>/dev/null || true
        wait "$release_pid" 2>/dev/null || true
        echo "Linux cold-start probe timed out before first frame marker" >&2
        exit 124
    fi
    if [ -n "$frame_seen_ns" ] && [ "$SECONDS" -ge "$report_deadline" ]; then
        kill "$release_pid" 2>/dev/null || true
        wait "$release_pid" 2>/dev/null || true
        echo "Linux cold-start probe timed out after first frame marker, before final report" >&2
        exit 124
    fi
    sleep 0.05
done
...
if [ -z "$report_seen_ns" ]; then
    echo "Linux cold-start probe exited before writing a valid final report" >&2
    wait "$release_pid" || true
    exit 1
fi
wait "$release_pid"

python3 - "$report_path" "$frame_marker_path" "$screenshot_path" "$started_ns" "$frame_seen_ns" "$report_seen_ns" "$max_seconds" <<'PY'
import json
from pathlib import Path
import sys

path = Path(sys.argv[1])
frame_marker_path = Path(sys.argv[2])
screenshot_path = Path(sys.argv[3])
started_ns, frame_seen_ns, report_seen_ns, max_seconds = map(int, sys.argv[4:])
if not path.is_file():
    raise SystemExit(f"cold-start probe did not write {path}")
if not frame_marker_path.is_file():
    raise SystemExit(f"cold-start probe did not write {frame_marker_path}")

result = json.loads(path.read_text(encoding="utf-8"))
frame_marker = json.loads(frame_marker_path.read_text(encoding="utf-8"))
if not frame_marker.get("frame_post_draw"):
