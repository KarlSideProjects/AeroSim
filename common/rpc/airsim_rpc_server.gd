...
func _dispatch_scene_object(message_id, method: String, params: Array) -> Array:
    ...
    if method == "simListSceneObjects":
        if not params.is_empty():
            return _error_response(message_id, "simListSceneObjects expects no parameters")
        ...
        if params.size() != 2 or typeof(params[0]) != TYPE_STRING or typeof(params[1]) != TYPE_BOOL:
            return _error_response(message_id, "simGetSegmentationObjectID expects object name and regex flag")
    ...
    var result: Dictionary = _scene_object_handler.call(method, params)
