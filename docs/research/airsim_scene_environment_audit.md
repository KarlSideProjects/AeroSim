...
Rain and fog are bounded visual state (`0..1`), wind is routed to the existing
native wind configuration, and time-of-day settings remain deterministic session
state. `reset` restores catalog lifecycle and environment state before restoring
the selected map wind baseline. Replay/dataset consumers can read the same
`EnvironmentState` and catalog callbacks used by RPC and the status diagram.
