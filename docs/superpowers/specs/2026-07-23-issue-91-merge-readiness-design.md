...
- Collision recovery uses the paired-counterfactual 500-substep response check rather than a height proxy: four scenes times 100 seeds times Angle/Acro, exactly one reset, collision energy at most 1.01 times pre-collision, frozen neutral/response commands, actual-motor response epsilon, FlightCore authority every step, finite bounded motor/controller state, and bitwise replay evidence.
