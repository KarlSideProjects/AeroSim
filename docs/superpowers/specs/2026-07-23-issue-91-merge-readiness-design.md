...
3. Shape Angle targets with `error_angle=wrap_pi(desired_angle-target_angle)`, `rate_cmd=clamp(error_angle/input_tc,-rate_max,+rate_max)`, `target_rate=move_toward(target_rate,rate_cmd,accel_max*dt)`, and `target_angle+=target_rate*dt`; crossing desired snaps `target_angle=desired_angle` and `target_rate=0`.  Shape Acro axes and Angle yaw into desired rates with the same rate/acceleration limits.  On the first armed substep, reset/collision reset, or either Acro↔Angle-family transition, synchronize target angle/rate to the measured state and clear I/D/previous-error/latches; Angle-to-Altitude preserves the shared family state.
...
The flight HUD always shows a compact Quad-X motor panel during flight; it is not an OSD-profile element and Minimal presets cannot hide it.  It is a nose-up top-down layout: `FL` / `FR` are the front row and `RL` / `RR` the rear row; this maps the frozen telemetry order `RR`, `FR`, `RL`, `FL` into physical positions rather than displaying index order.  Each cell displays the existing 30 Hz telemetry for that motor: actual thrust in N, speed converted from rad/s to RPM, and current in A.  A saturated motor is visibly distinguished without relying on colour alone.  The panel uses the existing telemetry snapshot only: it adds no native method, polling path, simulation mutation, or replay field.

The UI labels are localized.  Freshness uses the same per-vehicle `publish_count` and receipt-age contract as the status diagram.  Missing, malformed, non-finite, incomplete, or non-live snapshots, and any paused/error screen after an atomic failure, display an explicit unavailable/error state for all affected values rather than retaining live-looking data.  GUT verifies physical motor order, units, saturation state, unavailable/error state, freshness, and independence from OSD presets; Ubuntu headed acceptance verifies that the panel remains readable and visible in the flight HUD.
