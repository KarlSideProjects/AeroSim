...
**Current evidence:** The current branch contains the renderer-shared Terrain Range scene with two formal spawn markers, a Terrain3D launch area, route landmarks, and collision geometry. In Quick Fly and Demo Flight, its existing GPU grass is a visual-only cue for the native simulation's prevailing horizontal wind plus bounded turbulence: it samples at the authored field marker after an authoritative frame advance, smooths and phases from simulation time, and freezes while simulation is paused or reset is pending. It does not alter physical wind, sensors, replay, or the public NED/FRD/SI coordinate contract. Map UI wind presets provide the northwest-to-southeast prevailing cue; an explicitly supplied RPC steady-wind vector remains authoritative.

Terrain Range structural/runtime checks and the full 319-test GUT suite cover the shared materials, exclusion geometry, authoritative-wind path, pause/reset behavior, and particle bounds. At `2c16a3457f96a30be3b1998c6e83f51fe631879b`, the designated Ubuntu 26.04 reference runner passed G0.1 and G3.7 performance qualification for the unchanged 230,400-particle field in both effects-off and effects-on configurations (14,400 samples each; p99 1.395 ms and 1.382 ms, respectively, against a 3.000 ms limit). Fresh-XDG headed acceptance produced Light and Severe captures; Codex's provisional review found varied grass, intact materials, clean hard-surface exclusions, and no visible clipping or popping. This is automated provisional evidence only: no human visual or usability review was requested before CAP-006. Full production visual acceptance remains incomplete.
