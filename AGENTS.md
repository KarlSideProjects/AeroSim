...
Use the project Codex configuration in `.codex/config.toml`: implementation work defaults to `gpt-5.6-terra` with medium reasoning effort. For a bounded, ambiguous, high-value implementation question, the main agent may ask the read-only `sol_advisor` custom agent, which uses `gpt-5.6-sol` with high reasoning effort. The main agent remains responsible for checking the answer and making the decision. If the selected model is unavailable, report that fact; do not silently claim that a different model was used.
